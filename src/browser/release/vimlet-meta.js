...
(function() {
  ...
  generator.parseTemplate = function(template, callback, data, context) {
    ...
    // Regex
    generator.__regex = new RegExp(generator.__escapeRegExp(generator.__tagOpen) + "(?:(?!" + generator.__escapeRegExp(generator.__tagOpen) + ")[\\s\\S])*" + generator.__escapeRegExp(generator.__tagClose) + "(\\r\\n|\\r|\\n){0,1}", "g");
    ...
    sandbox.__parseTemplate = function(t) {
      ...
      if (matches) {
        for (var i = 0; i < matches.length; i++) {
          ...
          evalResult.push(sandbox.__eval(matches[i], generator.__getBasePath(t)));
        }
      }
  ...
  generator.__cleanMatch = function(match) {
    ...
    // Remove tags
    match = match.substring(generator.__tagOpen.length, match.length - generator.__tagClose.length).trim();

    // Echo shortcut
    if (match.startsWith(generator.__tagEcho)) {
      match = "echo(" + match.substring(generator.__tagEcho.length, match.length).trim() + ");";
    }
